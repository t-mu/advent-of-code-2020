...
describe('calendar3_part1', () => {
  ...
  describe('when given an array of movement pattern coordinates', () => {
    describe('and the array is empty', () => {
      it('should throw no movement pattern error', () => {
        expect(() => calendar3_part1(calendar3ExampleInput, [])).toThrowError(ErrorMessage.NO_MOVEMENT);
      });
    });
    describe('and there`s only one pattern', () => {
    ...
    describe('and all patterns are valid', () => {
      it('should return the multiplication of trees encountered by each pattern', () => {
        expect(calendar3_part1(calendar3ExampleInput, [{ x: 1, y: 1 }, { x: 3, y: 1 }])).toEqual(14);
      });
    });
  });
});
